...
function startMusic() {
  ...
  droneFreqs.forEach((freq, i) => {
    ...
    lp.type = 'lowpass';
    lp.frequency.value = 200 + i * 60;
    osc.connect(lp); lp.connect(g); g.connect(master);
    osc.start();
    if (i === 0) musicNodes.droneOsc = osc;
  });
  ...
  bassOsc.start();
  musicNodes.bassGain = bassGain;

  // Rhythmic pulse: bass pumps every beat
  const BPM = 80;
  ...
  function pulseBass() {
    ...
    bassGain.gain.exponentialRampToValueAtTime(0.001, t + beatInterval * 0.9);
    beatCount++;
    // Every 4th beat: accent
    if (beatCount % 4 === 0) {
      bassGain.gain.setValueAtTime(0.28, t);
    }
    musicNodes.beatTimeout = setTimeout(pulseBass, beatInterval * 1000);
  }
  pulseBass();

  // Tension tick (hi-hat style noise)
  ...
  function playTick() {
    ...
    tg.gain.setValueAtTime(0.06 + (level - 1) * 0.01, audioCtx.currentTime);
    tg.gain.exponentialRampToValueAtTime(0.001, audioCtx.currentTime + 0.04);
    src.connect(hp); hp.connect(tg); tg.connect(master);
    src.start();
    tickCount++;
    // Faster ticking at higher levels
    const tickRate = Math.max(250, beatInterval * 1000 / 2 - level * 20);
    musicNodes.tickTimeout = setTimeout(playTick, tickRate);
  }
  playTick();

  // Ambient cave hum (low rumble)
  ...
  rumbleOsc.start();
  musicNodes.rumbleGain = rumbleGain;
  ...
  padOsc.start(); padLfo.start();
}

function updateMusicIntensity() {
  if (!musicNodes.master || !audioCtx) return;
  // Intensify music as level rises
  const intensity = Math.min(1, (level - 1) / 8);
  const t = audioCtx.currentTime;
  musicNodes.master.gain.linearRampToValueAtTime(0.18 + intensity * 0.2, t + 2);
  if (musicNodes.rumbleGain)
    musicNodes.rumbleGain.gain.linearRampToValueAtTime(0.08 + intensity * 0.15, t + 2);
}
...
function stopMusic() {
  ...
  clearTimeout(musicNodes.tickTimeout);
  if (musicNodes.master) {
    musicNodes.master.gain.linearRampToValueAtTime(0, audioCtx.currentTime + 1);
    setTimeout(() => { try { musicNodes.master.disconnect(); } catch(e){} }, 1200);
  }
  musicNodes = {};
}
